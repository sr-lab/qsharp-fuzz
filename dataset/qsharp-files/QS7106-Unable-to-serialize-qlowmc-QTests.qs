...
namespace QTests.LowMC
{
    ...
    operation SBox(_a: Bool[], in_place: Bool, costing: Bool) : Result[]
    {
    ...
    operation SBoxLayer(_a: Bool[], blocksize: Int, sboxes: Int, in_place: Bool, costing: Bool) : Result[]
    {
    ...
    operation AffineLayer(_a: Bool[], round: Int, id: Int, costing: Bool) : Result[]
    {
        mutable blocksize = 0;
        if (id == 0)
        {
    ...
    operation KeyExpansion(_a: Bool[], round: Int, id: Int, costing: Bool) : Result[]
    {
        mutable blocksize = 0;
        if (id == 0)
        {
    ...
    operation Round(in_state: Bool[], round_key: Bool[], round: Int, id: Int, costing: Bool) : Result[]
    {
        mutable blocksize = 0;
        let sboxes = 10;
        if (id == 0)
        ...
        using ((in_s, ancillas, rk) = (Qubit[blocksize], Qubit[3*sboxes], Qubit[blocksize]))
        {
            for (i in 1..blocksize)
            {
                SetFromBool(in_state[i-1], in_s[i-1]);
                SetFromBool(round_key[i-1], rk[i-1]);
            }
    ...
    operation Encrypt(_key: Bool[], _message: Bool[], id: Int, costing: Bool) : Result[]
    {
        mutable blocksize = 0;
        mutable rounds = 0;
        let sboxes = 10;
        ...
        using ((key, message, ciphertext) = (Qubit[blocksize], Qubit[blocksize], Qubit[blocksize]))
        {
            for (i in 1..blocksize)
            {
                SetFromBool(_key[i-1], key[i-1]);
                SetFromBool(_message[i-1], message[i-1]);
            }
    ...
    operation GroverOracle (_key: Bool[], _plaintexts: Bool[], target_ciphertext: Bool[], pairs: Int, id: Int, costing: Bool) : Result
    {
        mutable blocksize = 0;
        mutable rounds = 0;
        let sboxes = 10;
        ...
        using ((key, success, plaintext) = (Qubit[blocksize], Qubit(), Qubit[blocksize*pairs]))
        {
            ...
            {
                SetFromBool(_key[i], key[i]);
            }
            for (j in 0..(pairs-1))
            {
                for (i in 0..(blocksize-1))
                {
                    SetFromBool(_plaintexts[blocksize*j + i], plaintext[blocksize*j + i]);
                }
            }
