...
namespace QUtilities
{
    ...
    operation SetFromBool(desired: Bool, q1: Qubit) : Unit {
        Set(BoolAsResult(desired), q1);
    }
    ...
    operation LPAND (in_1: Qubit, in_2: Qubit, outp: Qubit, costing: Bool) : Unit
    {
        body (...)
        {
            ...
            {
                // New built-in AND
                ApplyLowDepthAnd(in_1, in_2, outp);
            }
        }
        adjoint auto;
    }
    ...
    operation AND(control1 : Qubit, control2 : Qubit, target : Qubit) : Unit {
        ...
        adjoint (...) {
            ...
            if (IsResultOne(M(target))) {
                ...
                CNOT(control1, control2);
                X(target);
                R(PauliZ, 0.3, control2); // hack for correct depth
                Adjoint R(PauliZ, 0.3, control2); // hack for correct depth
            }
        }
    }
    ...
    operation ApplyToPairOfIndices<'T>(op : (('T, 'T) => Unit is Adj + Ctl), idx1 : Int, idx2 : Int, register : 'T[]) : Unit // is Adj + Ctl
    {
        body(...)
        {
            op(register[idx1], register[idx2]);
        }
        controlled adjoint auto;
    }
}
